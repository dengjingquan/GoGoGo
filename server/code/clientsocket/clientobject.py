...
class CClientObj(threading.Thread):
    ...
    def run(self):
        while self.m_bConnected:
            try:
                pubdefines.FormatPrint("recv")
                sData = self.m_oSocket.recv(1024*1024)
                pubdefines.FormatPrint("recv success")
            except :
    ...
    def Recv(self, sData):
        pubdefines.FormatPrint("aaaaaaaaa")
        self.m_sData += sData
        pubdefines.FormatPrint(len(sData))
        pubdefines.FormatPrint("bbbb")
        pubdefines.FormatPrint(sData)
        while True:
            ...
            pubdefines.FormatPrint(dPackData)
